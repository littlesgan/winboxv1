<#
.SYNOPSIS
Winbox - Windows Enhancement Utility - irm "https://github.com/memstechtips/Winbox/blob/main/Winbox.ps1" | iex

.DESCRIPTION
Winbox is a PowerShell GUI application designed to optimize and customize Windows 10 and 11 systems. 
...
using namespace WinboxExtensions
...
$SCRIPT:LogPath = "$env:TEMP\Winbox-Log_$(Get-Date -Format 'yyyyMMdd-HHmmss').log"
...
# Global paths
$SCRIPT:ConfigPath = Join-Path $env:ProgramFiles "Winbox\Config"
...
function Start-Log {
    ...
    @"
==== Winbox Log - $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') ====
...
function Backup-Registry {
    ...
    try {
        ...
        $backupDir = Join-Path $env:ProgramFiles "Winbox\Backups\REG"
        ...
        $isFirstRun = -not (Get-ChildItem -Path $backupDir -Filter "WinboxBackup_FirstRun.reg" -ErrorAction SilentlyContinue)
        
        # Set backup filename based on whether it's first run
        if ($isFirstRun) {
            $backupFile = Join-Path -Path $backupDir -ChildPath "WinboxBackup_FirstRun.reg"
        }
        else {
            $timestamp = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
            $backupFile = Join-Path -Path $backupDir -ChildPath "WinboxBackup_$timestamp.reg"
        }
        ...
        if (-not $isFirstRun) {
            ...
            $backups = Get-ChildItem -Path $backupDir -Filter "WinboxBackup_*.reg" | 
            Where-Object { $_.Name -ne "WinboxBackup_FirstRun.reg" } |
...
# Function to install Microsoft Store and Remove Winbox Removal script and scheduled task
function Install-Store {
    try {
        # First, update BloatRemoval.ps1 to remove Store entry
        $bloatRemovalPath = "$env:ProgramFiles\Winbox\Scripts\BloatRemoval.ps1"
...
# Function to install Microsoft Edge and Remove Winbox Removal script and scheduled task
function Install-MicrosoftEdge {
    try {
        ...
        # Delete Winbox Edge tasks and scripts
        ...
        $scriptPath = "$env:ProgramFiles\Winbox\Scripts\EdgeRemoval.ps1"
...
# Function to install OneDrive and Remove Winbox Removal script and scheduled task
function Install-OneDrive {
    try {
        ...
        # Delete Winbox OneDrive tasks and scripts
        ...
        $scriptPath = "$env:ProgramFiles\Winbox\Scripts\OneDriveRemoval.ps1"
        if (Test-Path $scriptPath) {
            Remove-Item -Path $scriptPath -Force -ErrorAction SilentlyContinue
...
# Function to install Xbox and Remove Winbox Removal script and scheduled task
function Install-Xbox {
    try {
        $bloatRemovalPath = "$env:ProgramFiles\Winbox\Scripts\BloatRemoval.ps1"
...
function Initialize-RemoveAppsButton {
    ...
    $text.Text = "Check all apps you don't want on your PC. Windows might try to add them back later, but Winbox will keep removing them for you."
...
$EdgeRemovalScript = {
    ...
    # Source: Winbox (https://github.com/memstechtips/Winbox)"
...
function Remove-Edge {
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
        ...
        try {
            ...
            Register-ScheduledTask -TaskName "Winbox\EdgeRemoval" `
...
- Disable OneDrive features system-wide`n`nAre you sure you want to continue?" -Title "Warning" -Buttons "YesNo" -Icon "Warning"
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
        ...
        try {
            ...
            Register-ScheduledTask -TaskName "Winbox\OneDriveRemoval" `
                -Action $action `
                -Trigger $trigger `
                -User "SYSTEM" `
                -RunLevel Highest `
...
function Remove-SelectedBloatware {
    ...
    if ($bloatScriptContent.Count -gt 4) {
        ...
        $scriptPath = "$env:ProgramFiles\Winbox\Scripts\BloatRemoval.ps1"
        if (-not (Test-Path -Path (Split-Path -Parent $scriptPath))) {
            New-Item -ItemType Directory -Path (Split-Path -Parent $scriptPath) -Force
        }
        ...
        Register-ScheduledTask -TaskName "Winbox\BloatRemoval" `
            -Action (New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-File `"$scriptPath`"") `
            -Trigger (New-ScheduledTaskTrigger -AtStartup) `
            -User "SYSTEM" `
            -RunLevel Highest `
...
<LineBreak />- Last used voice activation setting
    ...
    Foreground="{DynamicResource PrimaryTextColor}"
    Margin="10,0,0,10">
    <Run Text="Winbox is a PowerShell GUI application designed to optimize and customize Windows 10 and 11 systems." />
    <LineBreak />
    ...
    Foreground="{DynamicResource PrimaryTextColor}"
    Margin="10,10,0,10"
    Text="Create Winbox Desktop Shortcut" />
                            <Button
...
$CloseButton.Add_Click({
        ...
        if ($showDialog) {
            $heart = [char]0x2764
            $response = Show-MessageBox -Message "Thanks for using Winbox! $heart

Thank you for  useing winbox
Click 'NO' to Close! 
Click yes for discord
" `
                -Title "Close Winbox" `
                -Buttons "YesNo" `
                -Icon "Information"
...
$CreateShortcutButton.Add_Click({
    try {
        ...
        $shortcutPath = Join-Path $desktopPath "Winbox.lnk"

        $WshShell = New-Object -ComObject WScript.Shell
        $Shortcut = $WshShell.CreateShortcut($shortcutPath)
        $Shortcut.TargetPath = "powershell.exe"
        $Shortcut.Arguments = "-ExecutionPolicy Bypass -Command `"Start-Process powershell -ArgumentList '-NoProfile -ExecutionPolicy Bypass -Command `"`"irm \`"https://github.com/memstechtips/Winbox/raw/main/Winbox.ps1\`" | iex`"`"' -Verb RunAs`""
...
$agreement = Show-MessageBox -Message "Welcome to Winbox!
...
try {
    ...
    if ($backupPath) {
        Show-MessageBox -Message "Welcome to Winbox!
